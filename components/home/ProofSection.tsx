...
export function ProofSection() {
  return (
    <section className="on-light bg-bone py-16 text-inkText sm:py-24" aria-labelledby="proof-heading">
      <div className="mx-auto max-w-6xl px-4">
        {/* Shashank's story */}
        <Reveal>
          <h2 id="proof-heading" className="max-w-2xl font-display text-3xl leading-tight sm:text-4xl">
            {STORY.headline}
          </h2>
        </Reveal>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.name} delay={i * 0.05}>
              <figure className="flex h-full flex-col rounded-2xl border border-inkText/10 bg-white/60 p-6">
                <blockquote className="flex-1 text-[15px] leading-relaxed">&ldquo;{t.quote}&rdquo;</blockquote>
                <figcaption className="mt-4 border-t border-inkText/10 pt-3 text-sm">
                  <span className="font-semibold">{t.name}</span>
                  <span className="text-inkText/60"> · {t.place}</span>
                  <p className="mt-0.5 font-medium text-ember">{t.result}</p>
                </figcaption>
              </figure>
            </Reveal>
          ))}
